  ...
  return (
    <SafeAreaView style={tw`flex-1 bg-[#f8fafc]`}>
      <ScrollView style={tw`flex-1`} showsVerticalScrollIndicator={false}>
        <HistoryHeader totalSpend={userProfile?.totalSpend || 0} totalInvoices={userProfile?.totalInvoices || 0} />
      ...
      <View style={tw`absolute bottom-0 left-0 right-0 bg-white border-t border-slate-100 py-2 px-4 flex-row justify-around items-center shadow-lg`}>
        ...
        <TouchableOpacity style={tw`items-center flex-1 py-1`}>
          ...
          <Text style={tw`text-[10px] text-[#00b894] font-bold mt-0.5`}>Lịch sử</Text>
        </TouchableOpacity>
        <TouchableOpacity style={tw`items-center flex-1 py-1`} onPress={() => onNavigate('profile')}>
